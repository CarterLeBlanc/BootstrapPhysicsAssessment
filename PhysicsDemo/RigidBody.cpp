...
void RigidBody::resolveCollision(RigidBody* other, glm::vec2 contact, glm::vec2* collisionNormal)
{
	...
	//Find the radii from axis to application of force
	...
	//Ignore the collision if they are moving apart
	if (velocity <= otherVelocity) 
	{
		return;
	}

	//Find the effective mass at contract point for each object
	float mass = 1.0f / (1.0f / m_mass + (otherRadius * otherRadius) / other->m_moment);
